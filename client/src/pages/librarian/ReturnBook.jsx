...
const ReturnBook = ({ setActiveSection }) => {
      ...
      try {
        ...
        setError(
          err.response?.data?.message ||
            "Failed to fetch students or active loans"
        );
    ...
    if (formData.studentId) {
      const booksForStudent = [
        ...activeLoans.loans.filter(
          (loan) => loan.studentId?._id?.toString() === formData.studentId
        ),
        ...activeLoans.issuedBooks.filter(
  ...
  const handleChange = (e) => {
    ...
    if (name === "studentName") {
      ...
      setFormData((prev) => ({
        ...
        type: selectedBook
          ? activeLoans.loans.some((l) => l._id === selectedBook._id)
            ? "loan"
            : "issuedBook"
          : "",
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(
        `Book returned successfully from ${formData.studentName || "student"}!`
      );
      setFormData({
        studentName: "",
        studentId: "",
